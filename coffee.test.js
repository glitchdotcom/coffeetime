...
test('check for user should return true if the user list already contains the user', () => {
    ...
    expect(coffee.checkForUser(mockSlackUser, mockJsonUsers)).toBe(true);
    expect(coffee.checkForUser(mockSlackUser, mockJasonUsers2)).toBe(false);
...
test('add user to data should return the data with the new user', () => {
  ...
  expect(coffee.checkForUser(mockSlackUser, newData)).toBe(true);
